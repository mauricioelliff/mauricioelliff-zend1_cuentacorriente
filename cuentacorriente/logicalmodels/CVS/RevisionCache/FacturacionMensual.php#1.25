...
class FacturacionMensual extends ClaseBaseAbstracta
{
    ...
    public function generarFacturacionMensual( 
                        ...
                        $fechaDeFinDeCobros=false, 
                        $sedeIdAProcesar=0,
                        $escribirEnLogSiYaEstabaCobrado=true,
                        $alumnosId=false
                    )
    {
        //$this->flashMessenger->clearMessages();
        $this->_logIniciar($sedeIdAProcesar);
        $this->_logEscribir( "\n\n Iniciando facturación mensual sede $sedeIdAProcesar, fecha $fechaDeInicioDeCobros \n" );
        
        $fechas = $this->getFechasDeCobro( $fechaDeInicioDeCobros, $fechaDeFinDeCobros );
        $this->_logEscribir( "\n Fechas: ". arrays_asociativoToHtml($fechas) );
        if( is_array( $fechas ) ){
            ...
            $this->_logEscribir( "\n Fechas incorrectas o fecha tope alcanzada. Fechas: ". arrays_asociativoToHtml($fechas)  );
            $this->_logCerrar();
            return false;
        }
        
        // items que en algún momento se facturaron o deberán facturarse, de cada estudiante:
        ...
        foreach( $aCobrar as $alumnos_id => $cobrosAlAlumno ){
            ...
            foreach( $cobrosAlAlumno as $evscxa_id => $cobrar ){
                ...
                $mov = array(
                        ...
                        // 'motivo'            => null, lo pone RegistradorDeMovimiento()
                ...
                if( $alta === TRUE ){
                    ...
                    if( is_array($alta) && 
                        ...
                        $yaRegistrados++;
                        if( !$escribirEnLogSiYaEstabaCobrado ){
                            continue;
                        }
                    }
        ...
        $logStringFile = file_get_contents( $this->_pathArchivosTxtDelProceso.$this->_fileLogProceso );
        
        return ( $alta && $cobrosNoAplicables==0 )? TRUE : $logStringFile;
    }
    
    // distribución de coberturas
    private function _distribuirCreditos( $sedeIdAProcesar = null )
    {
        $sedeIdAProcesar = ($sedeIdAProcesar>0)? $sedeIdAProcesar : null;
        
        $CoberturaOperaciones = 
                new CoberturaOperaciones(
                                        $sedeIdAProcesar, 
                                        $this->cuentaCorrienteColeccion, 
                                        $FuncionesSobreEVAs = null // no se necesita en esto
                                        );
        $CoberturaOperaciones->distribuirTodosLosCreditosLibres( $sedeIdAProcesar );
    ...
    public function getFechasDeCobro( $fechaDeInicioDeCobros, $fechaDeFinDeCobros )
    {
        ...
        if( !validateDate( $fechaDeInicioDeCobros, 'Y-m-d') ||
            ...
            $fechaDeInicioDeCobros = date('Y-m-d',restaDias($fechaDeInicioDeCobros,1) );
    ...
    public function getFechaInicioDeCobros()
    {
        ...
        if( file_exists( $file ) ){
            ...
            // le sumo 1 día
            return date( 'Y-m-d', sumaDias( $fecha, 1 ) );
        }
        return false;
         * 
         */
    ...
    private function _logIniciar($sedeIdAProcesar)
    {
