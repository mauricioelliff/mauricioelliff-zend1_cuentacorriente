...
class PromoPlataformaMesBonificado extends Promocion implements PromocionInterfase
{
    ...
    public function operarPromo( $fechaSolicitud )
    {
        ...
        $meses = $this->_getAnioMesParaAplicarPromo();
        if( $meses === TRUE ){
        ...
        /* MONTO PARCIAL SOBRE EL SEGUNDO MES. ESTO POR AHORA LO QUITO Y DEJO COMENTADO.
         ...
         */
        /*
        if( $this->_aplicarPromoCostoCeroAEsteMes( $meses ) ){
            $this->addColeccionMensajes("1 Mes Plataforma gratis \n");
            $mesesRestantes = $this->_getMesesPosterioresAPromoCostoCero( $meses, $mesesPotencialesParaRecibirPromo );
        ...
        return $this->_aplicarPromoCostoCeroAEsteMes( $meses );
    }
    
    private function _getMesesPosterioresAPromoCostoCero( $anioMesParaAplicarCostoCero, array $mesesPotencialesParaRecibirPromo )
    {
    ...
    private function _aplicarPromoCostoCeroAEsteMes( $anioMesArray )
    {
        foreach( $anioMesArray as $anioMes ){
     ...
     private function _evaSetearValor( $valor, $fechaDesde, $fechaHasta, $motivo, $informarAContable=true, $closureCheck=false )
    {
        if( is_null($fechaDesde) || empty($fechaDesde) 
           || !key_exists(substr($fechaDesde,0,4), $this->_PlataformasAlumnoPorAnio ) ){
           return false;
        }
     ...
     */
    private function _getAnioMesParaAplicarPromo()
    {
        $cantMesesAAplicarPromo = count( $this->_mesesParaPromo() );
        ...
        while( true ){
            ...
            $mes= (($mes+1)>12 )? '01' : str_pad($mes+1,'0',2);
            $anio= ($mes=='01' )? ($anio+1) : $anio;
                    continue;
                }
            }
            // Obtengo los evas:
            ...
            // Trato de aplicar la promo al eva correspondiente
            foreach( $this->_preciosPlataforma[$anio] as $values ){
                $precioAnioMes = substr($values['fecha_inicio_calculado'],0,7 );

                if( $precioAnioMes == "$anio-$mes" ){
                    // Check, si ya tiene aplicada la promo.
                    if( $values['valor_modificado_motivo'] == $this->_motivo ){         
                        return true; // ya la tenía aplicada. no hace falta aplicar más.
                    }
                    if( in_array($values['valor_modificado_motivo'], $this->_getMotivosQueNoPermitenAcceso() ) ){
                        $resultado[]= $precioAnioMes;
                    }
                    if( $cantMesesAAplicarPromo == count($resultado) ){
                         break 2;
                    }
                }
            }
    ...
    {
        if( empty($anio) || !key_exists($anio,$this->_preciosPlataforma) ){
            return false;
        }
        foreach( $this->_preciosPlataforma[$anio] as $values ){
    ...
    /*
     * DEBERIA DEVOLVER LA CANTIDAD DE MESES QUE SE DEBE APLICAR PROMO
     * 
